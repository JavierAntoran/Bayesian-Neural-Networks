...
class bayes_linear_LR_2L_gauss(nn.Module):
    def __init__(self, input_dim, output_dim, nhid, prior_sig):
        super(bayes_linear_LR_2L_gauss, self).__init__()
        ...
        self.bfc3 = BayesLinear_local_reparam(n_hid, 2*output_dim, self.prior_sig)
    ...
    def forward(self, x, sample=False):
        ...
        mu = y[:, :self.output_dim]
        sigma = F.softplus(y[:, self.output_dim:])

        return mu, sigma, tlqw, tlpw

    def sample_predict(self, x, Nsamples):
        # Just copies type from x, initializes new vector
        predictions_mu = x.data.new(Nsamples, x.shape[0], self.output_dim)
        predictions_sigma = x.data.new(Nsamples, x.shape[0], self.output_dim)
        tlqw_vec = np.zeros(Nsamples)
        tlpw_vec = np.zeros(Nsamples)

        for i in range(Nsamples):
            mu, sigma, tlqw, tlpw = self.forward(x, sample=True)
            predictions_mu[i] = mu
            predictions_sigma[i] = sigma
            tlqw_vec[i] = tlqw
            tlpw_vec[i] = tlpw

        return predictions_mu, predictions_sigma, tlqw_vec, tlpw_vec


class BBP_Bayes_Net_LR_gauss(BaseNet):
    ...
    def __init__(self, lr=1e-3, channels_in=3, side_in=28, cuda=True, classes=10, batch_size=128, Nbatches=0,
        ...
        super(BBP_Bayes_Net_LR_gauss, self).__init__()
    ...
    def create_net(self):
        ...
        self.model = bayes_linear_LR_2L_gauss(input_dim=self.channels_in * self.side_in * self.side_in, output_dim=self.classes,
    ...
    def fit(self, x, y, samples=1):
        ...
        if samples == 1:
            mu, sigma, tlqw, tlpw = self.model(x)
            mlpdw = -diagonal_gauss_loglike(y, mu, sigma).sum()
            ...
            for i in range(samples):
                mu, sigma, tlqw, tlpw = self.model(x, sample=True)
                sigma = sigma.clamp(min=1e-2)
                # mlpdw_i = F.cross_entropy(out, y, reduction='sum')
                mlpdw_i = -diagonal_gauss_loglike(y, mu, sigma).sum()
    ...
    def eval(self, x, y, train=False):
        ...
        mu, sigma, _, _ = self.model(x)

        loss = -diagonal_gauss_loglike(y, mu, sigma).sum()
    ...
    def sample_eval(self, x, y, Nsamples, logits=True, train=False):
        ...
        mu, sigma, _, _ = self.model.sample_predict(x, Nsamples)

        mu_out = mu.mean(dim=0, keepdim=False)
        sigma_out = sigma.mean(dim=0, keepdim=False)
        loss = -diagonal_gauss_loglike(y, mu_out, sigma_out).sum()

        return loss.data, mu.data, sigma.data
